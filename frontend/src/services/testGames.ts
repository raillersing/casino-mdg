...
export function getTestGames(token: string) {
  return request<{ currency: "SIM"; results: InstantGame[] }>(
    "test-games/catalog/",
    { token },
  );
}
...
export function playTestGame(
  ...
  return request<InstantPlay>(`test-games/${slug}/plays/`, {
    token,
    method: "POST",
...
export function getTestDraws(token: string) {
  return request<{ currency: "SIM"; results: TestDraw[] }>("test-draws/", {
    token,
  });
}

export function enterTestDraw(
  token: string,
  slug: string,
  numbers: number[],
  idempotencyKey: string,
  ...
  return request<{
    ...
    created: boolean;
  }>(`test-draws/${slug}/entries/`, {
    token,
    method: "POST",
    headers: { "Idempotency-Key": idempotencyKey },
    body: JSON.stringify({ numbers, idempotency_key: idempotencyKey }),
  });
}
...
export function getTestActivity(token: string) {
  ...
  }>("test-games/activity/", { token });
}

export function simulateTestDraw(token: string, slug: string) {
  return request<TestDraw & { created: boolean }>(
    `test-draws/${slug}/result/`,
