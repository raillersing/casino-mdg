...
import { ArrowUpRight, ChevronRight, Clock3, Dices, Flame, Plus, ShieldCheck, Sparkles, Ticket } from 'lucide-react'
...
export function HomePage() {
  ...
  return <div className="page-stack home-page">
    <section className="hero-panel">
      ...
      <div className="hero-stats"><div><strong>SIM</strong><span>{t('home.activePlayers')}</span></div><div><strong>04</strong><span>{t('home.availableGames')}</span></div><div><strong>24/7</strong><span>{t('home.openTables')}</span></div></div>
    </section>
    ...
    <section className="quick-games-promo"><div className="quick-games-copy"><span className="eyebrow gold"><Sparkles size={13}/> {t('home.quickGames.eyebrow')}</span><h2>{t('home.quickGames.title')}</h2><p>{t('home.quickGames.body')}</p><div className="quick-games-tags"><span><Dices size={14}/> {t('home.quickGames.instant')}</span><span><Ticket size={14}/> {t('home.quickGames.draws')}</span><span><ShieldCheck size={14}/> {t('home.quickGames.simOnly')}</span></div></div><Link to="/games/test" className="button button-gold">{t('home.quickGames.cta')} <ArrowUpRight size={16}/></Link></section>
