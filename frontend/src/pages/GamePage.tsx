...
export function GamePage() {
  ...
  const sendMessage = async () => {
    ...
    try { const sent = await sendTableMessage(tableId, message.trim(), accessToken); setChat((current) => [...current, sent]); setMessage(''); setSocialError('') } catch (error) { setSocialError(error instanceof Error ? error.message : 'Message refusé.') }
  }

  const inviteFriend = async () => {
    if (!tableId || !accessToken) return
    try { const result = await createTableInvitation(tableId, accessToken); const link = `${window.location.origin}/game/${gameType}/${tableId}?invite=${result.token}`; await navigator.clipboard?.writeText(link); setInvite(link); setSocialError('') } catch (error) { setSocialError(error instanceof Error ? error.message : 'Invitation impossible.') }
  }
