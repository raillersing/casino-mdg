...
class OTPFlowTests(TestCase):
    ...
    def test_otp_is_single_use_after_successful_verification(self):
        response = self.client.post(
            "/api/v1/auth/otp/request/", {"phone": "0340000001"}, format="json"
        )
        payload = {
            "phone": "0340000001",
            "code": response.data["dev_code"],
    ...
    def test_shared_device_fingerprint_requires_review(self):
        first = User.objects.create_user(
            email="device1@mdg.local", phone="+261340000018", display_name="One"
        )
        second = User.objects.create_user(
            email="device2@mdg.local", phone="+261340000019", display_name="Two"
        )
        self.client.force_authenticate(first)
        self.assertEqual(
            self.client.post(
                "/api/v1/auth/devices/",
                {"device_id": "device-a", "fingerprint": "fp-1"},
                format="json",
            ).status_code,
            201,
        )
        ...
        response = self.client.post(
            "/api/v1/auth/devices/",
            {"device_id": "device-b", "fingerprint": "fp-1"},
            format="json",
        )
        self.assertEqual(response.status_code, 409)
        self.assertTrue(response.data["review_required"])
        self.assertEqual(UserDevice.objects.filter(fingerprint="fp-1").count(), 2)
