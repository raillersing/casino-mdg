...
def can_access(table, user):
    return TableSeat.objects.filter(table=table, user=user).exists() or table.created_by_id == user.pk
...
class TableChatView(APIView):
    ...
    def get(self, request, table_id):
        ...
        if table is None: return Response({"detail": "Table introuvable."}, status=404)
        if table is False: return Response({"detail": "Accès refusé."}, status=403)
        messages = ChatMessage.objects.filter(table=table, is_hidden=False).select_related("author")[:100]
        return Response({"results": [{"id": item.pk, "author": item.author.display_name, "body": item.body, "created_at": item.created_at.isoformat()} for item in messages]})
    ...
    def post(self, request, table_id):
        ...
        if not body or len(body) > 500: return Response({"detail": "Message invalide."}, status=400)
...
class TableInvitationView(APIView):
    ...
    def post(self, request, table_id):
        try: table = GameTable.objects.get(pk=table_id)
