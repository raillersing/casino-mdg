...
class SocialTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email="social@mdg.local", phone="+261340000011", display_name="Social")
        self.other = User.objects.create_user(email="other@mdg.local", phone="+261340000012", display_name="Other")
        self.table = GameTable.objects.create(table_code="social-001", name="Social", game_type="poker", created_by=self.user)
    ...
    def test_invitation_requires_table_access(self):
        response = self.client.post(f"/api/v1/social/tables/{self.table.pk}/invitations/")
        self.assertEqual(response.status_code, 201); self.assertTrue(response.data["token"])
        outsider = APIClient(); outsider.force_authenticate(self.other)
        self.assertEqual(outsider.post(f"/api/v1/social/tables/{self.table.pk}/invitations/").status_code, 403)
