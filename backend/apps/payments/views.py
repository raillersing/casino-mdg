...
class PaymentWebhookView(APIView):
    ...
    def post(self, request, provider):
        if provider not in dict(WebhookInboxEvent.PROVIDERS):
            return Response({"detail": "Opérateur inconnu."}, status=404)
        ...
        if not event_id or not event_type:
            return Response({"detail": "Événement webhook invalide."}, status=400)
        ...
        if not created and (
            event.event_type != event_type or event.payload != request.data
        ):
            return Response(
                {
        ...
        if created:
            ...
            if intent_id and event_type in status_by_event:
                ...
                ).update(status=status_by_event[event_type])
...
class PaymentIntentView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            amount = 0
        if (
            provider not in dict(WebhookInboxEvent.PROVIDERS)
            or direction not in dict(PaymentIntent.DIRECTIONS)
            or amount <= 0
            or not key
            or len(key) > 160
        ):
        ...
        if not created and intent.user_id != request.user.pk:
            return Response({"detail": "Clé d'idempotence déjà utilisée."}, status=409)
        return Response(
            {
                "id": str(intent.id),
