...
class PaymentWebhookTests(TestCase):
    ...
    def test_reused_event_id_with_different_payload_is_rejected(self):
        client = APIClient()
        first_payload = {"event_id": "evt-conflict", "event_type": "payment.succeeded", "amount": 100}
        first_body = json.dumps(first_payload, separators=(",", ":")).encode()
        first_signature = hmac.new(settings.PAYMENT_WEBHOOK_SECRET.encode(), first_body, hashlib.sha256).hexdigest()
        self.assertEqual(client.post("/api/v1/payments/webhooks/mvola/", first_payload, format="json", HTTP_X_WEBHOOK_SIGNATURE=first_signature).status_code, 201)
        second_payload = {**first_payload, "amount": 999}
        second_body = json.dumps(second_payload, separators=(",", ":")).encode()
        second_signature = hmac.new(settings.PAYMENT_WEBHOOK_SECRET.encode(), second_body, hashlib.sha256).hexdigest()

        response = client.post("/api/v1/payments/webhooks/mvola/", second_payload, format="json", HTTP_X_WEBHOOK_SIGNATURE=second_signature)
