...
class DailyMissionTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="mission@mdg.local", phone="+261340000031", display_name="Mission"
        )
    ...
    def test_missions_are_derived_from_today_results_and_claimed_once(self):
        ...
        first = self.client.post(
            "/api/v1/games/missions/", {"key": "win_daily"}, format="json"
        )
        ...
        self.assertEqual(
            WalletTransaction.objects.filter(user=self.user, type="bonus").count(), 1
        )

    def test_unfinished_mission_cannot_be_claimed(self):
        response = self.client.post(
            "/api/v1/games/missions/", {"key": "play_daily"}, format="json"
        )
