...
def engine_headers(game_id, game_type, outcome, amount, metadata=None):
    signature = hmac.new(settings.GAME_ENGINE_RESULT_SECRET.encode(), game_result_signature_payload(game_id, game_type, outcome, amount, metadata or {}), hashlib.sha256).hexdigest()
...
class GameResultTests(TestCase):
    def test_win_is_credited_once_when_result_is_retried(self):
        ...
        headers = engine_headers(payload["game_id"], payload["game_type"], payload["outcome"], payload["amount"])
        first = client.post("/api/v1/games/results/", payload, format="json", **headers)
        second = client.post("/api/v1/games/results/", payload, format="json", **headers)
    ...
    def test_history_returns_personal_stats(self):
        ...
        for outcome, amount in (("win", 100), ("loss", 0), ("draw", 0)):
            payload = {"game_id": str(uuid.uuid4()), "game_type": "rami", "outcome": outcome, "amount": amount}
            client.post("/api/v1/games/results/", payload, format="json", **(engine_headers(payload["game_id"], payload["game_type"], outcome, amount) if outcome == "win" else {}))
    ...
    def test_win_without_engine_signature_is_rejected(self):
        user = User.objects.create_user(email="unsigned@mdg.local", phone="+261340000015", display_name="Unsigned")
        client = APIClient(); client.force_authenticate(user)
        response = client.post("/api/v1/games/results/", {"game_id": str(uuid.uuid4()), "game_type": "poker", "outcome": "win", "amount": 100}, format="json")
    ...
    def test_leaderboard_is_ranked_by_wins_then_winnings(self):
        ...
        client = APIClient()
        for _ in range(2):
            client.force_authenticate(first); payload = {"game_id": str(uuid.uuid4()), "game_type": "poker", "outcome": "win", "amount": 10}; client.post("/api/v1/games/results/", payload, format="json", **engine_headers(payload["game_id"], payload["game_type"], payload["outcome"], payload["amount"]))
        client.force_authenticate(second); payload = {"game_id": str(uuid.uuid4()), "game_type": "poker", "outcome": "win", "amount": 500}; client.post("/api/v1/games/results/", payload, format="json", **engine_headers(payload["game_id"], payload["game_type"], payload["outcome"], payload["amount"]))
