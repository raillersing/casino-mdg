...
def debit_simulation_entry(user, idempotency_key, amount, description, metadata=None):
    ...
    platform.balance += amount
    platform.save(update_fields=["balance", "updated_at"])
    entry = WalletTransaction.objects.create(transaction_code=f"SIM-PLAY-{user.pk}-{idempotency_key[-20:]}", user=user, type="game", direction="debit", amount=amount, currency_code="SIM", status="completed", source_account=player, destination_account=platform, idempotency_key=idempotency_key, description=description, metadata=metadata or {}, processed_at=timezone.now())
    LedgerEntry.objects.bulk_create([LedgerEntry(transaction=entry, account=player, entry_type="debit", amount=amount, balance_after=player.balance), LedgerEntry(transaction=entry, account=platform, entry_type="credit", amount=amount, balance_after=platform.balance)])
    return entry, True
